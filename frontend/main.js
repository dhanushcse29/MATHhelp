document.addEventListener("DOMContentLoaded", function () {
  ...
  async function loadAnnouncements() {
    ...
    data.forEach((a) => {
      ...
      div.innerHTML = `<strong>${a.creator?.username || "Admin"}:</strong> ${
        a.message
      ...
      .then((materials) => {
        fetch("/api/students")
              ...
              <div class="dashboard-section">
                ...
                <ul class="material-list">
                  ${materials
                    .map(
                      (m) =>
                        `<li>${m.title} <a href=\"/api/materials/download/${m._id}\">Download</a></li>`
                    )
                    .join("")}
                </ul>
              </div>
              <div class="dashboard-section">
                ...
                <ul class="student-list">
                  ${students
                    .map(
                      (s) =>
                        `<li>${s.username} <button data-username=\"${s.username}\" class=\"reset-btn\">Reset Password</button></li>`
                    )
                    .join("")}
                </ul>
              </div>
              ...
              <div class="dashboard-section">
                ...
                <ul class="material-list">
                  ${announcements
                    .map(
                      (a) =>
                        `<li>${a.message} <small>by ${
                          a.creator?.username || "Admin"
                        } (${new Date(
                          a.createdAt
          ...
          `);
                ...
                document.getElementById("cancel-dashboard-btn").onclick =
                  closeModal;
                if (user.role === "admin") {
                  ...
                  document.getElementById("create-student-form").onsubmit =
                    async (e) => {
                      e.preventDefault();
                      const fd = new FormData(e.target);
                  ...
                  document.querySelectorAll(".reset-btn").forEach((btn) => {
                    btn.onclick = async () => {
                      const newPassword = prompt(
                        "Enter new password for " + btn.dataset.username
                      );
                  ...
                  document.getElementById("announce-form").onsubmit = async (
                    e
                  ) => {
                    e.preventDefault();
                    const fd = new FormData(e.target);
